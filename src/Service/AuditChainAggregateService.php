...
namespace BlackCat\Database\Packages\AuditChain\Service;

use BlackCat\Core\Database;
use BlackCat\Database\Packages\AuditChain\Dto\AuditChainDto;
use BlackCat\Database\Packages\AuditChain\Mapper\AuditChainDtoMapper;
 ...
 */
final class AuditChainAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private AuditChainRepository $auditChainRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'audit_chain' => $this->auditChainRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\AuditChain\ModuleException("Record {$idStr} not found for locking.");
            }
