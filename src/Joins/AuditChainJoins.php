...
namespace BlackCat\Database\Packages\AuditChain\Joins;
...
final class AuditChainJoins {
    ...
    /**
     * FK: audit_chain -> audit_log
     * LEFT JOIN vw_audit_log AS $as ON $as.id = $alias.audit_id
     * @return array{0:string,1:array<string,mixed>}
     */
    public function joinAuditLog(string $alias = 't', string $as = 'j0'): array {
        [$alias, $as] = $this->assertAliasPair($alias, $as);
        return [' LEFT JOIN vw_audit_log AS ' . $as . ' ON ' . $as . '.id = ' . $alias . '.audit_id' . ' ', []];
    }
