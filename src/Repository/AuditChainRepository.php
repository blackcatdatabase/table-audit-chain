...
namespace BlackCat\Database\Packages\AuditChain\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\AuditChain\Definitions;
use BlackCat\Database\Packages\AuditChain\Criteria;
use BlackCat\Database\Packages\AuditChain\Dto\AuditChainDto as Dto;
use BlackCat\Database\Packages\AuditChain\Mapper\AuditChainDtoMapper as RowMapper;
...
final class AuditChainRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\AuditChain\Definitions::class; }
    ...
    private function doUpsert(array $row, bool $revive): void
    {
        ...
        $updCols = [ 'chain_name', 'prev_hash', 'hash' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [ 'audit_id' ] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\AuditChain\Definitions::table());
            return $bulk->upsertMany($rows, [ 'audit_id' ], [ 'chain_name', 'prev_hash', 'hash' ]);
        }
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\AuditChain\Dto\AuditChainDto|null */
    public function getByAuditId(int $auditId, bool $asDto = false): array|\BlackCat\Database\Packages\AuditChain\Dto\AuditChainDto|null {
        $row = $this->getByUnique([ 'audit_id' => $auditId ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\AuditChain\Mapper\AuditChainDtoMapper::fromRow($row);
    }
    public function existsByAuditId(int $auditId): bool {
        $where = 't.' . Ident::q($this->db, 'audit_id') . ' = :uniq_audit_id';
        return $this->exists($where, [ 'uniq_audit_id' => $auditId ]);
    }
    /** @return int|string|null */
    public function getIdByAuditId(int $auditId) {
        $row = $this->getByAuditId($auditId);
        return $row ? ($row['id'] ?? null) : null;
    }
    /** @return array<string,mixed>|\BlackCat\Database\Packages\AuditChain\Dto\AuditChainDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\AuditChain\Dto\AuditChainDto|null {
