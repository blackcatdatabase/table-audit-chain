...
use BlackCat\Database\Packages\AuditChain\Repository\AuditChainRepositoryInterface;
...
class AuditChainRepository implements AuditChainRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;

    /** @var mixed literal token for upsert keys (array or empty). */
    private mixed $tokenUpsertKeys = [ 'audit_id' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\AuditChain\Definitions::class);
          $bulk->upsertMany($rows, $helperKeys, [ 'chain_name', 'prev_hash', 'hash' ]);
          return count($rows);
        }
    ...
    public function getByAuditId(int $auditId, bool $asDto = false): array|\BlackCat\Database\Packages\AuditChain\Dto\AuditChainDto|null {
        return $this->getByUnique([ 'audit_id' => $auditId ], $asDto);
    }
    ...
    public function getIdByAuditId(int $auditId) {
        $row = $this->getByAuditId($auditId, false);
